...
object Nfa {
  ...
  * */
  ...
  def fromPrenex(str: String): Nfa[Int] = {
    ...
    var parti_componente = sir_de_parsat.split(' ')//impartim in token-uri
    ...
    if(str.equals(" "))
      return Nfa.apply(indice_stare," ",indice_stare+1)
    if(sir_de_parsat.last==' ')
      parti_componente=parti_componente:+" "
